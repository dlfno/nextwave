...
if command -v psql >/dev/null && [ -n "${DATABASE_URL:-}" ]; then
  psql "$DATABASE_URL" -q -c "DELETE FROM providers WHERE domain = '$DOMAIN';" 2>/dev/null
fi
...
if [ "${1:-}" = "--with-crawler" ]; then
  ...
  echo "     stats: $(get "/runs/$RID" | jq -c '.stats')"
  ...
  echo "     stats: $(get "/runs/$RID2" | jq -c '.stats')"
  ...
  eq "y es el mismo run_id" "$(echo "$E1" | jq -r '.run_id')" "$(echo "$E2" | jq -r '.run_id')"
else
