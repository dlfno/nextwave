...
const adversarialCases: readonly AdversarialCase[] = [
  ...
  ['different origin', POLICY_REASON_CODES.ORIGIN_NOT_ALLOWED,
    (input) => ({ ...input, checkout: { ...input.checkout, lineItems: [{ ...input.checkout.lineItems[0]!, originIata: 'LAX' }] } })],
  ['different destination', POLICY_REASON_CODES.DESTINATION_NOT_ALLOWED,
    (input) => ({ ...input, checkout: { ...input.checkout, lineItems: [{ ...input.checkout.lineItems[0]!, destinationIata: 'ODB' }] } })],
  ['different departure date', POLICY_REASON_CODES.DEPARTURE_DATE_NOT_ALLOWED,
    (input) => ({ ...input, checkout: { ...input.checkout, lineItems: [{ ...input.checkout.lineItems[0]!, departureDate: '2026-09-16' }] } })],
