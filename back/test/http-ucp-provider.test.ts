...
import {
  Ap2CredentialIssuer, ap2CheckoutHash, ap2CheckoutMandateSchema,
  ap2PaymentMandateSchema, ap2TransactionAuthorizationSchema,
...
describe('HTTP UCP NubeVia adapters', () => {
  ...
  beforeEach(async () => {
    ...
    platformIssuer = await Ap2CredentialIssuer.create(
      platformPrivateJwk, 'platform-test-key', 'urn:nextwave:test-platform',
    );
    const platformPublicJwk: JWK = { ...platformPrivateJwk, kid: 'platform-test-key', alg: 'ES256', use: 'sig' };
    delete platformPublicJwk.d;
    const app = await createNubeViaSimulator({
      privateJwk: await exportJWK(privateKey), platformKeys: [platformPublicJwk],
    });
    server = app.listen(0);
  ...
  it('discovers, refreshes, signs, verifies, and completes an authoritative checkout', async () => {
    ...
    const profile = await (await fetch(`${baseUrl}/.well-known/ucp`)).json() as {
    ...
    expect(offers[0]).toMatchObject({
      providerId: 'http-nubevia-merchant-api', merchantId: NUBEVIA_MERCHANT_ID,
      merchantProductId: 'NV-MEX-COR-145', unitPriceMinor: '14500', sourceType: 'MERCHANT_API',
    });
    ...
    const closedCheckout = ap2CheckoutMandateSchema.parse({
      vct: 'mandate.checkout.1', checkout_jwt: checkout.signedPayload, checkout_hash: checkoutHash,
      iat: Math.floor(Date.now() / 1_000), exp: Math.floor(expiresAt.getTime() / 1_000),
    });
    ...
    const authorization = ap2TransactionAuthorizationSchema.parse({
      type: 'delegate', format: 'dc+sd-jwt', delegate_payload: [closedCheckout, closedPayment],
    });
    const presentation = await platformIssuer.issueDelegation(authorization, expiresAt);
    const rejected = await fetch(`${baseUrl}/checkout-sessions/${checkout.providerCheckoutId}/complete`, {
      method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
        payment: { instruments: [{ id: 'bad', handler_id: 'mock', type: 'tokenized', selected: true,
