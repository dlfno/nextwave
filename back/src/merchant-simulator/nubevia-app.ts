...
const offerReferenceSchema = z
  .object({
    offerId: z.uuid(),
    merchantId: z.literal(NUBEVIA_MERCHANT_ID),
    merchantProductId: z.literal(MERCHANT_PRODUCT_ID),
    productId: z.uuid().nullable(),
    productName: z.string().min(1),
    category: z.literal("travel.flight"),
    discoveredUnitPriceMinor: z.string().regex(/^\d+$/),
    ...
    line_items: z
      .array(
        z
          .object({
            item: z
              .object({ id: z.literal(MERCHANT_PRODUCT_ID) })
              .passthrough(),
...
const checkoutCompleteSchema = z
  .object({
    payment: z
      .object({
        instruments: z
          .array(
            z
              .object({
                ...
                display: z.object({ description: z.string() }).optional(),
                credential: z
                  .object({ type: z.string().min(1), token: z.string().min(1) })
                  .passthrough(),
              })
              .passthrough(),
          )
...
export async function createNubeViaSimulator(
  options: NubeViaSimulatorOptions,
): Promise<Express> {
  ...
  app.get("/health", (_request, response) =>
    response.json({ status: "ok", merchant: "NubeVia" }),
  );
  app.get("/.well-known/jwks.json", (_request, response) =>
    response.json({ keys: [signer.publicJwk()] }),
  );
  app.get("/.well-known/ucp", (_request, response) =>
    response.json(businessProfile(signer.publicJwk())),
  );
  ...
  app.post("/merchant/v1/search", (request, response) => {
    const input = z
      .object({
        specification: searchSpecificationSchema,
    ...
    const matches =
      spec.origin.iata === "MEX" &&
      spec.destination.iata === "COR" &&
      spec.passengers === 1 &&
      spec.currency === "USD";
    const departureTime = `${spec.departureDate}T12:45:00Z`;
    response.json({
      offers: matches
        ? [
            {
              ...
              description:
                "Flexible economy fare discovered through the NubeVia merchant catalog.",
              category: "travel.flight",
              unitPriceMinor: DISCOVERY_PRICE_MINOR,
              currency: "USD",
  ...
  app.post("/merchant/v1/quotes", (request, response) => {
    const input = z
      .object({ offer: offerReferenceSchema, currentTime: z.iso.datetime() })
      .strict()
      .parse(request.body);
    ...
    const lineItem = {
      ...
      productName: "NubeVia Mexico City to Córdoba",
      category: "travel.flight" as const,
      originIata: "MEX" as const,
      destinationIata: "COR" as const,
      departureDate: input.offer.departureDate ?? "2026-09-15",
      quantity: 1,
      unitPriceMinor: LIVE_PRICE_MINOR.toString(),
      totalMinor: LIVE_PRICE_MINOR.toString(),
  ...
  app.post("/checkout-sessions", async (request, response) => {
      ...
      line_items: [
        {
          ...
          item: {
            ...
            title: "NubeVia Mexico City to Córdoba",
            price: LIVE_PRICE_MINOR,
          },
          quantity: input.line_items[0]!.quantity,
          totals: [
            { type: "subtotal", amount: LIVE_PRICE_MINOR },
            { type: "total", amount: LIVE_PRICE_MINOR },
          ],
        },
      ],
    ...
    checkouts.set(providerCheckoutId, {
      merchantOrderId: `NV-ORDER-${randomUUID()}`,
  ...
  app.post(
    "/checkout-sessions/:providerCheckoutId/complete",
    async (request, response) => {
      ...
      if (!checkout) {
        response
          .status(404)
          .json(ucpError("checkout_not_found", "Checkout does not exist"));
        return;
      }
      ...
      if (new Date(checkout.expiresAt) <= new Date()) {
        response
          .status(409)
          .json(ucpError("checkout_expired", "Checkout expired"));
        return;
      }
        ...
        response
          ...
          .json(
            ucpError(
              ...
              "Payment and checkout mandates differ",
            ),
          );
        return;
      }
      const verified = await verifyMandate(
        ...
        checkout,
        await platformKeys(options),
      );
      if (!verified.ok) {
        response
          .status(400)
          .json(
            ucpError(verified.code, "AP2 checkout mandate verification failed"),
          );
        return;
      }
      checkout.completedAt = new Date().toISOString();
      checkout.checkoutReceipt = await signer.signReceipt(
        ap2CheckoutReceiptSchema.parse({
          status: "Success",
          iss: "urn:nubevia:merchant",
          iat: Math.floor(new Date(checkout.completedAt).getTime() / 1_000),
          reference: ap2CredentialHash(input.ap2.checkout_mandate),
          order_id: checkout.merchantOrderId,
        }),
      );
      response.json(completedCheckout(checkout));
    },
    ...
    (
      ...
      if (error instanceof z.ZodError) {
        response
          .status(400)
          .json(
            ucpError("invalid_request", "Invalid UCP request", error.issues),
          );
        return;
      }
      response
        .status(500)
        .json(
          ucpError("internal_error", "NubeVia could not process the request"),
        );
    },
  );
  return app;
...
async function verifyMandate(
  ...
  try {
    ...
    if (!issuerJwt || !disclosure || terminator !== "")
    ...
    const issuerPayload = z
      .object({
        exp: z.number().int(),
        delegate_payload: z.array(z.object({ "...": z.string() })).length(1),
        _sd_alg: z.literal("sha-256"),
      })
      .passthrough()
      .parse(JSON.parse(Buffer.from(verified.payload).toString("utf8")));
    if (issuerPayload.exp <= Math.floor(Date.now() / 1_000))
      return { ok: false, code: "mandate_expired" };
    if (
      issuerPayload.delegate_payload[0]?.["..."] !==
    ...
    const expectedHash = createHash("sha256")
      .update(checkout.signedPayload, "utf8")
      .digest("base64url");
    if (
      checkoutMandate.checkout_jwt !== checkout.signedPayload ||
...
function completedCheckout(checkout: StoredCheckout) {
  return {
    ...
    status: "completed",
    order: {
      id: checkout.merchantOrderId,
      permalink_url: `https://nubevia.example/orders/${checkout.merchantOrderId}`,
    },
    completed_at: checkout.completedAt,
    ap2_receipt: checkout.checkoutReceipt && {
...
function ucpError(code: string, message: string, details?: unknown) {
  return {
    ...
    messages: [{ type: "error", code, message, details }],
  };
}
